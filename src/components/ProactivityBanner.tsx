      ...
      <div className="p-5">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            {data.lastRefresh && (
              <span className="text-[11px] text-zinc-600">refreshed {timeAgo(data.lastRefresh)}</span>
            )}
            <button
        ...
        <div className="grid grid-cols-3 gap-3">
          <Link href="/proactivity" className="group relative bg-zinc-800/50 hover:bg-zinc-800 rounded-xl p-4 transition-all border border-zinc-800 hover:border-zinc-700">
            ...
            <div className="text-xs text-zinc-500">Unread intel</div>
            {data.intel.unread > 0 && (
              <div className="mt-2.5 text-[10px] text-orange-400 font-medium">Review →</div>
            )}
            {data.intel.total > 0 && (
          ...
          <Link href="/proactivity" className="group relative bg-zinc-800/50 hover:bg-zinc-800 rounded-xl p-4 transition-all border border-zinc-800 hover:border-zinc-700">
            ...
            <div className="text-xs text-zinc-500">Suggestions pending</div>
            {data.suggestions.pending > 0 && (
              <div className="mt-2.5 text-[10px] text-orange-400 font-medium">Approve →</div>
            )}
          ...
          <Link href="/proactivity" className={`group relative rounded-xl p-4 transition-all border ${
            ...
            {isRunning && (
              <div className="mt-2.5 text-[10px] text-orange-400 font-medium">View →</div>
            )}
          </Link>
        </div>
      </div>
