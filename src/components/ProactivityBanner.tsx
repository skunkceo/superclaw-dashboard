...
export function ProactivityBanner() {
  ...
  const load = async () => {
    try {
      const [intelRes, sugRes, nightRes, itemsRes] = await Promise.all([
        ...
        fetch('/api/overnight'),
        fetch('/api/suggestions?status=pending&limit=5'),
      ]);
    ...
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 animate-pulse h-full">
        ...
        <div className="space-y-2">
          {[...Array(4)].map((_, i) => <div key={i} className="h-8 bg-zinc-800 rounded-lg" />)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border flex flex-col h-full ${
      isRunning
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold text-white">Proactivity</h2>
            {hasAttention && (
          ...
          <div className="flex items-center gap-2">
            {data.lastRefresh && (
              <span className="text-[11px] text-zinc-600 hidden sm:block">refreshed {timeAgo(data.lastRefresh)}</span>
            )}
            <button
        ...
        <div className="flex items-center gap-3 mb-4 text-xs">
          <span>
            <span className={data.intel.unread > 0 ? 'text-orange-400 font-medium' : 'text-zinc-400'}>{data.intel.unread}</span>
            <span className="text-zinc-600"> unread</span>
          </span>
          <span className="text-zinc-700">·</span>
          <span>
            <span className={data.suggestions.pending > 0 ? 'text-orange-400 font-medium' : 'text-zinc-400'}>{data.suggestions.pending}</span>
          ...
          {data.overnight.queuedCount > 0 && (
            <>
              <span className="text-zinc-700">·</span>
              <span>
                <span className="text-zinc-400">{data.overnight.queuedCount}</span>
                <span className="text-zinc-600"> queued tonight</span>
              </span>
            </>
          )}
        </div>
        ...
        <div className="flex-1 space-y-1.5 min-h-0">
          {items.length === 0 ? (
            <div className="py-6 text-center">
              ...
              <button onClick={handleRefresh} className="mt-1.5 text-xs text-orange-400 hover:text-orange-300 transition-colors">
                Refresh intel to generate
              </button>
            </div>
          ) : (
            items.map(item => (
              <Link
                key={item.id}
                href={`/proactivity?id=${item.id}`}
                className="flex items-start gap-2.5 p-2.5 rounded-lg bg-zinc-800/40 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 transition-all group"
              >
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-zinc-200 group-hover:text-white transition-colors line-clamp-1 font-medium">
                    {item.title}
                  </div>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="text-[10px] text-zinc-600">{CATEGORY_LABELS[item.category] || item.category}</span>
                    <span className="text-zinc-700">·</span>
                    <span className={`text-[10px] font-medium ${IMPACT_COLORS[item.impact] || 'text-zinc-500'}`}>
                      {item.impact} impact
                    </span>
                  </div>
                </div>
                <svg className="w-3 h-3 text-zinc-700 group-hover:text-zinc-500 flex-shrink-0 mt-0.5 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {items.length > 0 && (
          <div className="mt-3 pt-3 border-t border-zinc-800/60">
            <Link href="/proactivity" className="text-xs text-zinc-500 hover:text-orange-400 transition-colors">
              View all {data.suggestions.pending} suggestions →
            </Link>
          </div>
        )}
      </div>
