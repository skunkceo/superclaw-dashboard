      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/proactivity" className="text-xs text-zinc-500 hover:text-orange-400 transition-colors font-medium">Open →</Link>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-1.5 min-h-0">
          {items.length===0?(
            ...
            <Link key={item.id} href={`/proactivity?id=${item.id}`} className="flex items-start gap-2.5 p-2.5 rounded-lg bg-zinc-800/40 hover:bg-zinc-800 border border-zinc-800 hover:border-zinc-700 transition-all group">
        ...
        {items.length>0&&(
          <div className="mt-3 pt-3 border-t border-zinc-800/60">
            <Link href="/proactivity" className="text-xs text-zinc-500 hover:text-orange-400 transition-colors">View all {data.suggestions.pending} suggestions →</Link>
          </div>
        )}
      </div>
    </div>
