...
import { getAllIntelItems, createIntelItem, markAllIntelRead, getIntelStats } from '@/lib/db';
...
export async function GET(request: Request) {
  ...
  const items = getAllIntelItems({ category, unread, limit });
...
export async function PATCH(request: Request) {
  ...
  const { searchParams } = new URL(request.url);
  if (searchParams.get('action') === 'mark_all_read') {
    markAllIntelRead();
